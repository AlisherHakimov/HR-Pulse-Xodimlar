...
class UpdatePermissionSheet extends StatefulWidget {
  const UpdatePermissionSheet({super.key, required this.permission});

  final PermissionModel permission;

  static void show(BuildContext context, PermissionModel permission) =>
      showModalBottomSheet(
        context: context,
        isScrollControlled: true,
        useSafeArea: true,
        builder: (context) => UpdatePermissionSheet(permission: permission),
      );
...
class _UpdatePermissionSheetState extends State<UpdatePermissionSheet> {
  ...
  initState() {
    super.initState();
    if (widget.permission.start != null && widget.permission.end != null) {
      _selectedDateRange = DateTimeRange(
        start: DateTime.parse(widget.permission.start ?? ''),
        end: DateTime.parse(widget.permission.end ?? ''),
      );
      bool isSingle = widget.permission.mode == 'SINGLE';
      _dateController.text =
          '${_dateFormat.format(DateTime.parse(widget.permission.start ?? ''))} ${!isSingle ? ' - ${_dateFormat.format(DateTime.parse(widget.permission.end ?? ''))}' : ''}';
      _fromController.text = DateFormat(
        'HH:mm',
      ).format(DateTime.parse(widget.permission.start ?? ''));
      _toController.text = DateFormat(
        'HH:mm',
      ).format(DateTime.parse(widget.permission.end ?? ''));
      _descriptionController.text = widget.permission.comment ?? '';
    }
  }
  ...
  // === FIXED _pickTime with proper validation ===
  Future<void> _pickTime(
    ...
    // Hozirgi tanlangan vaqtni o'qib olish
    ...
    TimeOfDay? maximumTime; // null bo'lsa → cheklov yo'q

    if (isFrom) {
      // From faqat ish boshlanishidan oldin bo'lmasligi kerak
      minimumTime = workStart;
    } else {
      // To uchun minimal vaqt — From dan 30 daqiqa keyin
      ...
      if (fromTime != null) {
        ...
        int minute = fromTime.minute + 30;
        if (minute >= 60) {
          hour += 1;
          minute -= 60;
        }
        minimumTime = TimeOfDay(hour: hour, minute: minute);
      } else {
        // Agar From hali tanlanmagan bo'lsa → wtStart + 30 daqiqa
        int hour = workStart.hour;
        int minute = workStart.minute + 30;
        if (minute >= 60) {
          hour += 1;
          minute -= 60;
        }
    ...
    // initialTime ni minimumTime dan past bo'lmasligini ta'minlash
    if (minimumTime != null) {
    ...
    final TimeOfDay? selected = await showDialog<TimeOfDay>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: _CustomTimePickerWidget(
          ...
          maximumTime: maximumTime, // null → cheklov yo'q
        ),
      ),
    );
    ...
    if (selected != null) {
      ...
      if (isFrom) {
        ...
        // Agar To tanlangan bo'lsa va endi From dan oldinroq yoki juda yaqin bo'lsa → tozalash
        if (_toController.text.isNotEmpty) {
          final toTime = _parseTime(_toController.text);
          if (toTime != null) {
            final fromMins = selected.hour * 60 + selected.minute;
            final toMins = toTime.hour * 60 + toTime.minute;
            if (toMins <= fromMins + 29) {
              _toController.clear();
            }
          }
        }
...
// === FIXED Custom Time Picker Widget ===
